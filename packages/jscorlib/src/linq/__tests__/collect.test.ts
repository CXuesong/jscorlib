...
describe("collect", () => {
  it("toArray", () => {
    ...
    expect(asLinq(arr).$_(_Collect.toArray())).toStrictEqual(arr);
  });
  it("toMap", () => {
    const arr = [1, 2, 3, "a", "b", "c"];

    const map1 = asLinq(arr).$_(_Collect.toMap(x => typeof x));
    expect(map1).toStrictEqual(new Map<string, string | number>([
      ["number", 3],
      ["string", "c"],
    ]));
    const map2 = asLinq(arr).$_(_Collect.toMap(x => x, x => typeof x));
    ...
    const hashMap1 = asLinq(arr).$_(_CollectHashMap.toHashMap(x => typeof x));
    expect(new Map(hashMap1)).toStrictEqual(map1);
    const hashMap2 = asLinq(arr).$_(_CollectHashMap.toHashMap(x => x, x => typeof x));
  ...
  it("toSet", () => {
    ...
    const set1 = asLinq(arr1).$_(_Collect.toSet());
    expect(set1).toStrictEqual(new Set(arr1));
    const set2 = asLinq(arr2).$_(_Collect.toSet());
    expect(set2).toStrictEqual(new Set([1, 2, 3, "a"]));

    const hashSet1 = asLinq(arr1).$_(_CollectHashSet.toHashSet());
    expect(new Set(hashSet1)).toStrictEqual(new Set(arr1));
    const hashSet2 = asLinq(arr2).$_(_CollectHashSet.toHashSet());
